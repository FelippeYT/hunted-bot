...
WEB_JSON = "monitor_data.json" # Arquivo que o HTML vai ler
...
online_cache = set()
last_deaths_ids = set()
web_events = [] # Cache para o site

# --- FUNÇÃO DE INTEGRAÇÃO COM O SITE ---
def update_web_json(name, level, event_type, details, status):
    """Adiciona evento ao monitor_data.json identificando se é Friend ou Hunted"""
    global web_events
    
    # Identifica o tipo do player para o Front-end
    player_type = "hunted"
    if name in friend_players:
        player_type = "friend"
    elif name in hunted_players:
        player_type = "hunted"
    else:
        player_type = "unknown" # Caso não esteja em nenhuma lista
    ...
    new_event = {
        ...
        "details": details,
        "status": status,
        "player_type": player_type # <--- NOVA CHAVE
    }
    
    web_events.insert(0, new_event)
    web_events = web_events[:100] # Aumentei para 100 para ter mais histórico no clique
    
    try:
        with open(WEB_JSON, "w", encoding="utf-8") as f:
            json.dump(web_events, f, ensure_ascii=False, indent=4)
    except Exception as e:
        print(f"❌ Erro ao salvar JSON do site: {e}")
...
async def get_api_data(url, endpoint_call):
    try:
        async with async_playwright() as p:
            ...
            # Timeout e wait ajustados para evitar travamentos
...
async def radar_principal():
    try:
        ...
        print(f"🔎 Radar: Verificando status... {datetime.now().strftime('%H:%M:%S')}")
        ...
        if data and 'players' in data:
            ...
            for p in current_online_list:
                ...
                if is_hunted or is_friend:
                    ...
                    if name not in online_cache:
                        ...
                        update_web_json(name, level, "status", "Entrou no jogo", "on")
                    ...
                    if old_level != 0 and level != old_level:
                        ...
                        update_web_json(name, level, "up" if diff > 0 else "down", f"{status} (antes {old_level})", "on")
            ...
            for name in list(online_cache):
                if name not in current_online_names:
                    ...
                    is_friend = name in friend_players
                    if is_hunted or is_friend:
                        update_web_json(name, "---", "status", "Saiu do jogo", "off")
                        target_channel = ch_hunted if is_hunted else ch_friends
...
async def check_mortes():
    try:
        ...
        if data and 'deaths' in data:
            for d in data['deaths']:
                ...
                if (is_hunted or is_friend) and death_id not in last_deaths_ids:
                    ...
                    # WEB UPDATE
                    update_web_json(player_name, d.get('level'), "pvp" if is_player else "pve", f"Morto por {killer}", "off")
        ...
        print(f"❌ Erro no Loop de Mortes: {e}")
